...
Use `op: "list"` to discover peers. Default is running+idle+execution-unknown plus per-status/shown/truncated counts — never an unbounded parked name dump. `execution-unknown` means observation was lost, not success, failure, or cancellation. Pass `status: "parked"` for parked archaeology; optional `limit` bounds rows (default 32, max 100). Address peers by exact roster ID — NEVER invent names. `send` to a known parked id still revives it; `history://<id>` and `agent://<id>` stay readable.
