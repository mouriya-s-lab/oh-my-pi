...
- Acceptance harness scaffold under `fork-features/acceptance/` with the B1–B4 baseline captured against upstream `omp/18.1.17` at the fork's HEAD; C1–C13 dual-remote matrix left BLOCKED pending remote-deploy prerequisites (forked binary deployment, OMP SSH capability configuration on two independent hosts, remote model access verification, cross-version pair) per RFC §10.1's "keep BLOCKED, do not fabricate" rule ([mouriya-s-lab#14](https://github.com/mouriya-s-lab/oh-my-pi/issues/14)).
- Task/eval/workpool spawn parameters are now interpreted in one consistent execution domain: explicit `schema`, `tools`, `isolated`, `effort`, `workpool items` cross the RPC verbatim (`handle` remains as an eval-entry alias for `retainArtifacts`); implicit kernel/registry/session state stays local; conflicts (remote-blocked host tool, non-isolable remote workspace, strict-schema unsatisfied) fail loudly with a `ParamsError` code, never silently drop the field. `effort` is not locally resolved; `apply` targets only the remote cwd repo (no auto-mirror to local); cross-endpoint budget/timeout/depth/spawns share one cumulative ledger ([mouriya-s-lab#12](https://github.com/mouriya-s-lab/oh-my-pi/issues/12)).
- Cross-endpoint IRC now uses the same envelope, waiter, inbox, and broadcast rules as local IRC: verbatim envelope forwarding (no second `bus.send()`), waiter stays on the sender and matches by `from`, three-stage delivery separation (RPC ack ≠ inbox injection ≠ model reply), reply-drained barrier with `runStatusRevision`+`outboundWatermark`, dedup by `(generation, operationId)`, and `indeterminate` state for unconfirmed frames. Managed capability set flips `ircBidirectional` to `1` ([mouriya-s-lab#11](https://github.com/mouriya-s-lab/oh-my-pi/issues/11)).
- Added a directly invocable SSH endpoint factory in `fork-features/ssh-remote-backend/` with capability lookup, absolute-path or PATH-probe executable resolution, stdin-preserving argv, pre-session cwd/profile/role preparation, and single-peer release without terminating the shared SSH master. Task-tool `target.kind: "ssh"` dispatch remains explicitly rejected pending the endpoint execution-path migration ([mouriya-s-lab#10](https://github.com/mouriya-s-lab/oh-my-pi/issues/10)).
- Managed `--rpc-subagent` RPC now negotiates the full D4/D5 native-agent contract: a versioned capability set (14 flags), correlation/scope/generation/operation IDs, a distinct `RpcErrorCode` taxonomy, four lifecycle paths (cancelRun / terminate / idle-TTL park / unexpected-EOF cancel-then-drain), a 10 s / 30 s bidirectional heartbeat + receive-side lease, and a `resume` handshake that returns `still-owned` when the reference is still in flight. Legacy `--mode rpc` behaviour is unchanged (D4 + D5 full for [mouriya-s-lab#9](https://github.com/mouriya-s-lab/oh-my-pi/issues/9); real cross-version verification blocked pending [#14](https://github.com/mouriya-s-lab/oh-my-pi/issues/14)).
- Migrated `AgentRegistry`, `AsyncJobManager`, and the hub jobs/list surface onto the tagged `AgentEndpoint` contract from [#3](https://github.com/mouriya-s-lab/oh-my-pi/issues/3). `AgentStatus` and `AsyncJob.status` now include a distinct `execution-unknown` terminal for transport-lost peers; every exhaustive switch and hub renderer surfaces it explicitly rather than collapsing into `failed` or `cancelled`. A reply-drained barrier prevents "stopped but not replied" judgements from resolving until both the terminal run event and the run's reply-drained event have fired (D2 migration for [mouriya-s-lab#8](https://github.com/mouriya-s-lab/oh-my-pi/issues/8); Agent Hub polish and disk-persisted remote references remain honestly deferred to [#13](https://github.com/mouriya-s-lab/oh-my-pi/issues/13) and [#14](https://github.com/mouriya-s-lab/oh-my-pi/issues/14) with an explicit `blocked pending` note).
- Extended the RFC #1 D1 target contract across task (flat + batch per-item), eval `agent()`, and workpool creation. Missing `target` still resolves to local with the pre-existing default-agent substitution; explicit `target: { kind: "ssh", ... }` runs through a shared normalize→authorize→prepare/start gate before any local model preflight and today produces an explicit "remote execution not yet wired" error pending [#8](https://github.com/mouriya-s-lab/oh-my-pi/issues/8). No `authStorage`/`modelRegistry` snapshot ever crosses the prepare payload (D1 full-gate for [mouriya-s-lab#7](https://github.com/mouriya-s-lab/oh-my-pi/issues/7); remote endpoint dispatch lands under [#8](https://github.com/mouriya-s-lab/oh-my-pi/issues/8) and [#10](https://github.com/mouriya-s-lab/oh-my-pi/issues/10)).
...
- Added the `AgentEndpoint` four-method contract (`prepare` / `start` / `run` / `cancelRun` + `terminate`) with a passive `LocalAgentEndpoint` adapter that wraps an existing `AgentSession` without changing local execution. Its transport-only `execution-unknown` verdict is consumed by the D2 monitor/registry migration above (contract skeleton for [mouriya-s-lab#3](https://github.com/mouriya-s-lab/oh-my-pi/issues/3)).
- `--mode rpc --rpc-subagent` opts a session into a managed bootstrap that declares a `nativeAgent` block on the `ready` frame, fast-paths `get_state`/`abort`/`abort_bash` off the serial command queue, and initiates in-flight cancellation on stdin EOF instead of draining silently. Legacy `--mode rpc` behaviour is unchanged when the flag is absent (skeleton for [mouriya-s-lab#4](https://github.com/mouriya-s-lab/oh-my-pi/issues/4); extended by the full [#9](https://github.com/mouriya-s-lab/oh-my-pi/issues/9) contract above).
- Added `fork-features/ssh-remote-backend/`: a self-contained SSH launcher that looks up a known host via the existing SSH capability, builds an argv-only remote command (`--mode rpc --rpc-subagent`, no task text, stdin preserved), and returns a transport spec the managed RPC client from [#4](https://github.com/mouriya-s-lab/oh-my-pi/pull/21) can consume. Loopback integration verifies ready + `get_state` + `abort` on an isolated sshd (skeleton for [mouriya-s-lab#5](https://github.com/mouriya-s-lab/oh-my-pi/issues/5); the endpoint wiring lands under [#7](https://github.com/mouriya-s-lab/oh-my-pi/issues/7) and the heterogeneous matrix under [#10](https://github.com/mouriya-s-lab/oh-my-pi/issues/10)).
- Added the stage-1 close-gate integration driver (`fork-features/close-gate/`): composes the target validator, endpoint contract, managed bootstrap flag, and SSH remote backend into one real loopback-sshd run. Captures B1–B4 baseline frames verbatim, verifies `nativeAgent.protocolMajor === 1`, exercises `get_state`/`abort` on the live managed session, and reproduces the `execution-unknown` snapshot fanout after transport break through the test-only endpoint adapter. Task-tool → endpoint dispatch ([mouriya-s-lab#7](https://github.com/mouriya-s-lab/oh-my-pi/issues/7)), live snapshot consumers ([#8](https://github.com/mouriya-s-lab/oh-my-pi/issues/8)), and IRC transport ([#11](https://github.com/mouriya-s-lab/oh-my-pi/issues/11)) rows remain honestly blocked pending those slices (skeleton close-gate for [mouriya-s-lab#6](https://github.com/mouriya-s-lab/oh-my-pi/issues/6)).
